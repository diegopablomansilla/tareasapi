...
public class PuestoFindAllDAO {
	...
	protected Result query(DataBase dataBase) throws LoadSQLTemplateDataSourceException, SelectException, SQLException {
	...
	private Statement buildStatement(boolean count) throws LoadSQLTemplateDataSourceException {
		...
		if (count) {
			...
			statement.setSql("SELECT PUESTO, NOMBRE\nFROM\tPuestos\nWITH (NOLOCK) ORDER BY PUESTO");
		}
